...
PLANNER_SYSTEM = """당신은 수석 DB 아키텍트인 'Planner'입니다.
학업 자료 공유 및 일정 관리 커뮤니티를 위한 최적의 ERD를 DBML 형식으로 작성해야 합니다.
기존 7개 테이블 구조를 발전시켜 확장성과 논리적 완결성을 갖춘 설계를 제시하세요.
Reviewer들의 피드백을 받으면 이를 적극적으로 반영하여 개선된 ERD를 내놓아야 합니다.
반드시 DBML 코드 블록만 출력하세요."""

REVIEWER1_SYSTEM = """당신은 데이터 정규화 및 무결성 전문가 'Reviewer 1'입니다.
Planner가 만든 ERD를 다음 기준으로 평가하고 JSON 형식으로 응답하세요:
...
def extract_json(text: str) -> Dict:
    try:
        ...
        return {"score": 0, "feedback": f"JSON 파싱 실패: {text[:200]}..."}
...
def main():
    ...
    while True:
        ...
        current_erd = planner_res.stdout
        log_progress(round_num, "planner", current_erd)
