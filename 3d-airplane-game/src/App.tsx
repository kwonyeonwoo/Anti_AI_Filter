...
const AIRCRAFT_DATA = {
  stealth: {
    ...
    mass: 18000, thrust: 220000, wingArea: 42, cd0: 0.012, liftSlope: 5.5, scale: 3, camDist: 50, camHeight: 10, color: "#3498db"
  },
  airliner: {
    name: "747 JUMBO JET",
    url: "https://vazxmixjsiawhamurptp.supabase.co/storage/v1/object/public/models/airplane/model.gltf",
    mass: 330000, thrust: 1100000, wingArea: 510, cd0: 0.022, liftSlope: 4.2, scale: 0.8, camDist: 150, camHeight: 25, color: "#e74c3c"
  },
  propeller: {
    name: "CESSNA SKYHAWK",
    url: "https://vazxmixjsiawhamurptp.supabase.co/storage/v1/object/public/models/propeller-plane/model.gltf",
    mass: 1100, thrust: 15000, wingArea: 16, cd0: 0.03, liftSlope: 4.0, scale: 3, camDist: 40, camHeight: 8, color: "#f1c40f"
  }
};
...
const LoadingScreen = () => (
  <Html center>
    <div style={{ color: 'white', background: 'rgba(0,0,0,0.9)', padding: '30px 60px', borderRadius: '20px', textAlign: 'center', border: '2px solid #3498db', minWidth: '300px' }}>
      <h1 style={{ margin: 0, fontStyle: 'italic' }}>PREPARING FLIGHT</h1>
      <p style={{ opacity: 0.5 }}>LOADING 3D ASSETS...</p>
    </div>
  </Html>
);
...
const Aircraft = ({ type, scale }: { type: keyof typeof AIRCRAFT_DATA, scale: number }) => {
  ...
  // All market-assets models face +Z or have weird orientations. 
  // We normalize them here to face -Z (forward).
...
const NaturalWorld = () => {
  const mountains = useMemo(() => {
    return [...Array(200)].map((_, i) => ({
      id: i,
      pos: [(Math.random() - 0.5) * 150000, 0, (Math.random() - 0.5) * 150000] as [number, number, number],
      h: 1000 + Math.random() * 8000,
      r: 2000 + Math.random() * 5000,
...
const FlightEngine = ({ aircraftType, setTelemetry }: any) => {
  ...
  // Physics States (Unified World Space)
  const pos = useRef(new THREE.Vector3(0, 3000, 0));
  ...
  useFrame((_state, delta) => {
    ...
    const authority = Math.min(1.0, currentSpeed / 50); // Control surface effectiveness

    const pitchTorque = (keys.current['s'] ? 1.0 : (keys.current['w'] ? -1.0 : 0)) * authority * 4.0;
    const rollTorque = (keys.current['a'] ? 1.0 : (keys.current['d'] ? -1.0 : 0)) * authority * 7.0;

    // 2. Aerodynamic Moments & Damping
    const damping = 4.0 * (currentSpeed / 200);
    angularVel.current.x += (pitchTorque - angularVel.current.x * damping) * delta;
    angularVel.current.z += (rollTorque - angularVel.current.z * damping) * delta;
    ...
    const liftCoeff = THREE.MathUtils.clamp(0.4 + aoa * specs.liftSlope, -1.0, 1.8);
    const liftForce = upVec.clone().multiplyScalar(dynamicPressure * specs.wingArea * liftCoeff);

    // Drag (Parasitic + Induced)
    const dragCoeff = specs.cd0 + (liftCoeff * liftCoeff) / (Math.PI * 6);
    const dragForce = velNorm.clone().multiplyScalar(-(dynamicPressure * specs.wingArea * dragCoeff));

    // 5. Physics Integration (F=ma)
    ...
    // Heading Alignment (Weather-vaning)
    velocity.current.lerp(forward.clone().multiplyScalar(currentSpeed), 2.5 * delta);

    pos.current.add(velocity.current.clone().multiplyScalar(delta));
    if (pos.current.y < 20) { pos.current.y = 20; velocity.current.y = 0; }

    // 6. Update Mesh & Camera
    if (airplaneRef.current) {
      airplaneRef.current.position.copy(pos.current);
      airplaneRef.current.quaternion.copy(quat.current);
    }

    const currentG = liftForce.length() / (specs.mass * G_ACCEL);
    const camOffset = new THREE.Vector3(0, specs.camHeight, specs.camDist).applyQuaternion(quat.current);
    camera.position.lerp(pos.current.clone().add(camOffset), 0.12);
  ...
  return (
    <>
      ...
      <fog attach="fog" args={['#d0e7ff', 1000, 70000]} />
    </>
  );
};
    ...
    <div style={{ width: '100vw', height: '100vh', background: '#d0e7ff', overflow: 'hidden', color: 'white', fontFamily: 'Impact, sans-serif' }}>
      {!started ? (
        <div style={{ position: 'absolute', zIndex: 10, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle, #2c3e50 0%, #000 100%)' }}>
          ...
          <p style={{ letterSpacing: '15px', color: '#aaa', marginBottom: '60px' }}>NEXT-GEN FLIGHT SIMULATOR</p>
        ...
        <>
          <div style={{ position: 'absolute', top: '40px', left: '40px', zIndex: 5, background: 'rgba(0,10,20,0.85)', padding: '30px', borderRadius: '25px', border: '2px solid #3498db', minWidth: '320px', backdropFilter: 'blur(10px)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #333', paddingTop: '20px' }}>
              <span style={{ fontSize: '1.5rem', color: telemetry.g > 8 ? '#ff0000' : '#f1c40f' }}>{telemetry.g} G</span>
              <span style={{ fontSize: '1.5rem', color: Math.abs(telemetry.aoa) > 15 ? '#ff0000' : '#aaa' }}>{telemetry.aoa}° AoA</span>
            </div>
          </div>
