...
const StealthJet = ({ scale }: { scale: number }) => {
  ...
  useFrame(({ clock }) => { if (flame.current) flame.current.scale.setScalar(0.8 + Math.sin(clock.elapsedTime * 50) * 0.2); });
  return (
...
// --- FLIGHT DYNAMICS ENGINE (MOMENT-BASED) ---
const FlightEngine = ({ aircraftType, setTelemetry }: any) => {
  ...
  const angularVelocity = useRef(new THREE.Vector3(0, 0, 0)); // Pitch, Yaw, Roll rates
  const throttle = useRef(0.6); 

  const specs = useMemo(() => {
    return aircraftType === 'stealth'
      ? { mass: 20000, thrust: 191000, wingArea: 42.7, cd0: 0.015, liftSlope: 5.5, scale: 30, camDist: 180, camHeight: 25, inertia: [2.0, 3.0, 1.0] }
      : { mass: 300000, thrust: 1128000, wingArea: 525, cd0: 0.022, liftSlope: 4.2, scale: 18, camDist: 1200, camHeight: 180, inertia: [5.0, 8.0, 3.0] };
  }, [aircraftType]);
  ...
  useEffect(() => {
    ...
    camera.position.set(0, 2025, 200);
  ...
  useFrame((_state, delta) => {
    ...
    const authority = Math.min(1.0, currentSpeed / 50); // Sluggish at low speed

    let pitchTorque = (keys.current['s'] ? 1.0 : (keys.current['w'] ? -1.0 : 0)) * authority * 2.0;
    let rollTorque = (keys.current['a'] ? 1.0 : (keys.current['d'] ? -1.0 : 0)) * authority * 4.0;
    let yawTorque = 0; // Rudder placeholder
    ...
    // 3. AERODYNAMIC MOMENTS (Damping)
    // Air resists rotation. Higher speed = higher damping.
    const dampingBase = 3.0 * (currentSpeed / 200);
    angularVelocity.current.x += (pitchTorque - angularVelocity.current.x * dampingBase) * delta / specs.inertia[0];
    angularVelocity.current.z += (rollTorque - angularVelocity.current.z * dampingBase) * delta / specs.inertia[2];
    
    // 4. DIRECTIONAL STABILITY (Weather-vaning)
    // The plane's nose wants to follow the velocity vector.
    const aoa = -velNorm.dot(upVec);
    const slip = -velNorm.dot(rightVec);
    
    // Pitch/Yaw corrections due to relative wind
    angularVelocity.current.x += aoa * 10 * delta * (currentSpeed / 100);
    angularVelocity.current.y += slip * 10 * delta * (currentSpeed / 100);

    // 5. UPDATE QUATERNION (Apply Angular Velocity)
    ...
    // Lift
    const liftCoeff = THREE.MathUtils.clamp(0.4 + aoa * specs.liftSlope, -0.8, 1.6);
    const liftMag = dynamicPressure * specs.wingArea * liftCoeff;
    const liftForce = upVec.clone().multiplyScalar(liftMag);
    // Drag
    const inducedDragK = 1 / (Math.PI * 6 * 0.8);
    const dragCoeff = specs.cd0 + inducedDragK * (liftCoeff * liftCoeff);
    const dragForce = velNorm.clone().multiplyScalar(-(dynamicPressure * specs.wingArea * dragCoeff));
    ...
    if (pos.current.y < 10) { pos.current.y = 10; velocity.current.y = 0; }
    ...
    const currentSpeedKph = currentSpeed * 3.6;
    ...
    if (_state.clock.elapsedTime % 0.1 < 0.02) {
      setTelemetry({ 
        speed: Math.round(currentSpeedKph), 
...
const NaturalWorld = () => {
  const mountains = useMemo(() => [...Array(150)].map((_, i) => ({
    ...
    h: 1000 + Math.random()*5000, r: 2000 + Math.random()*3000, color: Math.random() > 0.6 ? '#3b5e2b' : '#5c4033'
  })), []);
  return (
    <group>
      {mountains.map(m => <mesh key={m.id} position={[m.pos[0], m.h/2 - 50, m.pos[2]]}><coneGeometry args={[m.r, m.h, 6]} /><meshStandardMaterial color={m.color} roughness={1} /></mesh>)}
      <mesh rotation={[-Math.PI/2, 0, 0]} position={[0, -100, 0]}><planeGeometry args={[400000, 400000]} /><meshStandardMaterial color="#2d5a27" /></mesh>
    ...
    <div style={{ width: '100vw', height: '100vh', background: '#d0e7ff', overflow: 'hidden', color: 'white', fontFamily: 'Impact, sans-serif' }}>
      {!started ? (
        <div style={{ position: 'absolute', zIndex: 10, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle, #1a2a3a 0%, #000 100%)' }}>
          ...
          <p style={{ letterSpacing: '15px', color: '#aaa', marginBottom: '50px' }}>PHYSICS-DRIVEN SIMULATION</p>
